...
def reset_test(layout):
    ...
    global light_status
    if layout not in CIRCUIT_LAYOUTS:
        print(f"Error: Layout '{layout}' not found in CIRCUIT_LAYOUTS.")
        return

    light_status = {
        label: {"checked": False, "wrong": False, "success": False}
        for label in CIRCUIT_LAYOUTS[layout].keys()
    }
...
def detect_lights(frame, layout, canvas):
    ...
    # Use this flag to avoid spamming the sound
    fail_sound_played = getattr(detect_lights, "fail_sound_played", False)

    for label, data in lights.items():
        if label not in light_status:
            print(f"Warning: Label '{label}' not found in light_status. Skipping.")
            all_correct = False
            continue
        ...
        if 0 <= y - roi_size < frame.shape[0] and 0 <= x - roi_size < frame.shape[1]:
            ...
            if brightness < 100:  # If brightness is too low
                all_correct = False
                continue
    ...
    # Update the UI before evaluating the final status
    canvas.update_idletasks()

    # Control sound playback with the `fail_sound_played` flag
    if all_failed:
        if not fail_sound_played:
            play_fail_sound()
            detect_lights.fail_sound_played = True  # Set the flag to avoid spamming
    else:
        detect_lights.fail_sound_played = False  # Reset the flag if the condition changes

    # Check if all lights are correct
...
def start_detection(layout, canvas):
    ...
    def detection_loop():
        ...
        while True:
            ...
            detect_lights(frame, layout, canvas)
